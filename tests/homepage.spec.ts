...
test.describe("Homepage", () => {
  ...
  test("subtitle is visible", async ({ page }) => {
    const subtitle = page.locator("text=Production-ready template with Auth, tRPC, Prisma, and more");
  ...
  test.describe("Feature Cards", () => {
    const featureCards = [
      { title: "Authentication", description: "NextAuth.js v5 with Credentials provider and Prisma adapter" },
  ...
  test.describe("Get Started Section", () => {
    ...
    test("instruction comments are visible", async ({ page }) => {
      await expect(page.locator("text=Copy .env.example to .env.local and configure")).toBeVisible();
